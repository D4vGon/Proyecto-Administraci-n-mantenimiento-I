...
def calcular_indicadores(equipo_id=None):
    conn = get_connection()
    if equipo_id:
        df_trabajos = pd.read_sql("SELECT * FROM trabajos WHERE equipo_id = ?", conn, params=(equipo_id,))
        df_paros = pd.read_sql("SELECT * FROM paros WHERE equipo_id = ?", conn, params=(equipo_id,))
    else:
        df_trabajos = pd.read_sql("SELECT * FROM trabajos", conn)
...
if menu == "📊 Dashboard":
    ...
    if df_activos.empty:
        ...
        if indicadores is not None:
            ...
            col1.metric("MTBF (H)", f"{indicadores['MTBF (H)']} h")
            col2.metric("MTTR (H)", f"{indicadores['MTTR (H)']} h")
            ...
            if not df_trab.empty:
                ...
                # Filtrar duraciones muy pequeñas (menos de 0.01 horas ≈ 36 segundos)
                ...
                if not df_trab.empty:
                    ...
                    st.info("No hay registros con duración significativa (mayor a 0.01 horas).")
            else:
...
# =========================================================
    ...
    with t_hist:
        ...
        if not df_trabajos_hist.empty:
            # Convertir a datetime y formatear para mostrar sin microsegundos
            df_trabajos_hist['fecha_inicio'] = pd.to_datetime(df_trabajos_hist['fecha_inicio'], errors='coerce')
            df_trabajos_hist['fecha_fin'] = pd.to_datetime(df_trabajos_hist['fecha_fin'], errors='coerce')
            df_trabajos_hist = df_trabajos_hist.dropna(subset=['fecha_inicio', 'fecha_fin'])
            # Filtros
            ...
            # Filtros de fecha opcionales (valor None = sin filtrar)
            ...
            # Formatear fechas para mostrar sin microsegundos
            df_trabajos_hist['fecha_inicio'] = df_trabajos_hist['fecha_inicio'].dt.strftime('%Y-%m-%d %H:%M:%S')
            df_trabajos_hist['fecha_fin'] = df_trabajos_hist['fecha_fin'].dt.strftime('%Y-%m-%d %H:%M:%S')
        ...
        if not df_paros_hist.empty:
            df_paros_hist['inicio_paro'] = pd.to_datetime(df_paros_hist['inicio_paro'], errors='coerce')
            df_paros_hist['fin_paro'] = pd.to_datetime(df_paros_hist['fin_paro'], errors='coerce')
            df_paros_hist = df_paros_hist.dropna(subset=['inicio_paro', 'fin_paro'])
            if 'filtro_eq' in locals() and filtro_eq:
                df_paros_hist = df_paros_hist[df_paros_hist['equipo_id'].isin(filtro_eq)]
            # Formatear fechas
            df_paros_hist['inicio_paro'] = df_paros_hist['inicio_paro'].dt.strftime('%Y-%m-%d %H:%M:%S')
            df_paros_hist['fin_paro'] = df_paros_hist['fin_paro'].dt.strftime('%Y-%m-%d %H:%M:%S')
